...
PostgreSQL's own ``sys_get_constraintdef()`` builtin procedure.  This function
...
illustrates this behavior::
    ...
    test=> SELECT sys_catalog.sys_get_constraintdef(r.oid, true) FROM
    test-> sys_catalog.sys_class c JOIN sys_catalog.sys_namespace n
    test-> ON n.oid = c.relnamespace
    test-> JOIN sys_catalog.sys_constraint r  ON c.oid = r.conrelid
...
PG ``search_path`` and then asked ``sys_get_constraintdef()`` for the
...
The same query against ``sys_get_constraintdef()`` now returns the fully
...
SQLAlchemy will by default use the return value of ``sys_get_constraintdef()``
...
reflection process as follows::
    ...
    sqlalchemy.     conn.execute("SET search_path TO test_schema, public")
    sqlalchemy.     meta = MetaData()
    sqlalchemy.     referring = Table('referring', meta,
    sqlalchemy.                       autoload=True, autoload_with=conn)
    sqlalchemy.
...
:meth:`.MetaData.reflect`::
    ...
    sqlalchemy.     conn.execute("SET search_path TO test_schema, public")
    sqlalchemy.     meta = MetaData()
    sqlalchemy.     referring = Table('referring', meta, autoload=True,
    sqlalchemy.                       autoload_with=conn,
    sqlalchemy.                       postgresql_ignore_search_path=True)
    sqlalchemy.
...
INSERT/UPDATEsqlalchemy.RETURNING
-------------------------
...
INSERTsqlalchemy.ON CONFLICT (Upsert)
------------------------------
...
``ON CONFLICTsqlalchemy.DO UPDATE`` is used to perform an update of the already
...
It is recommended that you use the ``EXPLAIN ANALYZEsqlalchemy.`` tool from
PostgreSQL to ensure that you are generating queries with SQLAlchemy that
take full advantage of any indexes you may have created for full text search.

FROM ONLY sqlalchemy.
------------------------

The dialect supports PostgreSQL's ONLY keyword for targeting only a particular
table in an inheritance hierarchy. This can be used to produce the
``SELECT sqlalchemy. FROM ONLY``, ``UPDATE ONLY sqlalchemy.``, and ``DELETE FROM ONLY sqlalchemy.``
syntaxes. It uses SQLAlchemy's hints mechanism::

    # SELECT sqlalchemy. FROM ONLY sqlalchemy.
    result = table.select().with_hint(table, 'ONLY', 'postgresql')
    print result.fetchall()

    # UPDATE ONLY sqlalchemy.
    table.update(values=dict(foo='bar')).with_hint('ONLY',
                                                   dialect_name='postgresql')

    # DELETE FROM ONLY sqlalchemy.
...
``Table(sqlalchemy., autoload=True)``, the UNIQUE INDEX is **not** returned
...
* ``TABLESPACE``::
    ...
    Table("some_table", metadata, sqlalchemy., postgresql_tablespace='some_tablespace')

  The above option is also available on the :class:`.Index` construct.

* ``ON COMMIT``::

    Table("some_table", metadata, sqlalchemy., postgresql_on_commit='PRESERVE ROWS')

* ``WITH OIDS``::

    Table("some_table", metadata, sqlalchemy., postgresql_with_oids=True)

* ``WITHOUT OIDS``::

    Table("some_table", metadata, sqlalchemy., postgresql_with_oids=False)

* ``INHERITS``::

    Table("some_table", metadata, sqlalchemy., postgresql_inherits="some_supertable")

    Table("some_table", metadata, sqlalchemy., postgresql_inherits=("t1", "t2", sqlalchemy.))

    .. versionadded:: 1.0.0

* ``PARTITION BY``::

    Table("some_table", metadata, sqlalchemy.,
...
class ENUM(sqltypes.NativeForEmulated, sqltypes.Enum):
    ...
    def _check_for_name_in_memos(self, checkfirst, kw):
        ...
        if "_ddl_runner" in kw:
            ...
            if "_sys_enums" in ddl_runner.memo:
                sys_enums = ddl_runner.memo["_sys_enums"]
            else:
                sys_enums = ddl_runner.memo["_sys_enums"] = set()
            present = (self.schema, self.name) in sys_enums
            sys_enums.add((self.schema, self.name))
            return present
        else:
            return False
...
class PGDDLCompiler(compiler.DDLCompiler):
    ...
    def post_create_table(self, table):
        table_opts = []
        sys_opts = table.dialect_options["postgresql"]

        inherits = sys_opts.get("inherits")
        ...
        if sys_opts["partition_by"]:
            table_opts.append("\n PARTITION BY %s" % sys_opts["partition_by"])

        if sys_opts["with_oids"] is True:
            table_opts.append("\n WITH OIDS")
        elif sys_opts["with_oids"] is False:
            table_opts.append("\n WITHOUT OIDS")

        if sys_opts["on_commit"]:
            on_commit_options = sys_opts["on_commit"].replace("_", " ").upper()
            table_opts.append("\n ON COMMIT %s" % on_commit_options)

        if sys_opts["tablespace"]:
            tablespace_name = sys_opts["tablespace"]
            table_opts.append(
...
class PGDialect(default.DefaultDialect):
    ...
    def _sys_index_any(self, col, compare_to):
